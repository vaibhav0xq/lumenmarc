Source: https://docs.base.org/base-chain/specs/reference/b20
Title: 
Fetched: 2026-09-07T11:07:28.911Z
...
`seizeWithMemo(from, to, amount, memo)` transfers balance from `from` to `to` and emits `Transfer`, `Memo`, and `Seized`. It is gated by `SEIZE_ROLE`, skips allowance and transfer policies, requires `from` to be denied by `SEIZE_HOLDER_POLICY`, and requires `to` to be authorized by `SEIZE_RECEIVER_POLICY`.
...
- B20 token standard: conceptual introduction to B20 and when to use it
- Generated reference
- B20 changelog